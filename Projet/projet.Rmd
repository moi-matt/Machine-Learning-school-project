...
Certains jeunes joueurs peuvent jouer à des postes très différents. Notre objectif est de pouvoir conseiller le poste qui conviendrait le mieux à ces jeunes joueurs en utilisant l'intelligence artificielle.

Pour cela, nous avons utilisé le jeu de données de FIFA 21 afin de pouvoir entrainer nos modèles.

```{r}
players=read.csv("FIFA.csv",encoding = "utf-8")
```
...
```{r}
ggplot(players_3)+aes(x=Finishing,y=Position,fill=Position)+geom_boxplot()+theme_classic()
```
...
```{r}
#ggpairs(players_3,mapping = aes(color=Position))
```


```{r}
p1 <- ggplot(data = players_3,
             mapping = aes(x = Crossing, y = Finishing,
                           colour = Position)) +
  geom_point() +
  geom_smooth(method = "loess") +
  labs(x = "Crossing", y = "Finishing") +
  theme_bw() +
...
```{r}
p2 <- ggplot(data = players_3,
             mapping = aes(x = Strength, y = Stamina,
                           colour = Position)) +
  geom_point() +
  geom_smooth(method = "loess") +
  labs(x = "Strength", y = "Stamina") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.title = element_blank())
...
single_perceptron<-function(x_train, activation='sigmoid', final_units=1){
    model = keras_model_sequential() 
    model %>% layer_dense(units=final_units, activation=activation, input_shape=length(x_train[1,]))
return(model)
}
two_layers<-function(x_train, activation='sigmoid', final_units=1){
    model = keras_model_sequential() 
    model %>% layer_dense(units=as.integer(length(x_train[1,])/2), activation=activation, input_shape=length(x_train[1,])) %>% layer_dense(units=final_units, activation=activation)
return(model)
}
three_layers<-function(x_train, activation='sigmoid', final_units=1){
    model = keras_model_sequential() 
    model %>% layer_dense(units=length(x_train[1,]), activation=activation, input_shape=length(x_train[1,])) %>% layer_dense(units=as.integer(length(x_train[1,])/2), activation=activation) %>% layer_dense(units=final_units, activation=activation)
return(model)
}
```

```{r}
compile_fit<-function(fun, activation, x=x_train,y=y_train, loss="binary_crossentropy", final_units=1, epochs=30, verbose=0, batch_size=5, validation_split=0.3){
  # Permet de compiler et d'exécuter un modèle
  model=fun(x,activation,final_units)
  model %>% compile(loss=loss,optimizer="adam",metrics='accuracy')
  summary(model)
  history=model%>% fit(x=x, y=y, epochs=epochs, batch_size=batch_size,validation_split=validation_split, verbose=verbose)
...
confusionMatrix(pred.rad,reference =na.omit(test)$Position)
```
...
```{r}
pred.tree <- predict(train.mod.tree,tree_test_data)
```
...
Attack=c("LW", "LF", "RW","RF", "CF", "ST")
Defense=c("RB", "RWB", "LB", "LWB", "CB")
Midfielder=c("CAM", "CM", "CDM", "LM", "RM","CAM")
...
ne code ne peut pas être exécuté quand on a un facteur de 3 levels. 

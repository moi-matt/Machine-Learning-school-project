...
```
On obtient plus de 98% d'accuracy tout comme la régression logistique.
...
pred.rad = predict(train.mod.rad, test)
...
On garde le modèle svm linéaire car ce dernier est plus précis. On garde le modèle linéaire pour vérifier sur nos tests.
...
```{r}
compar1=ggplot(data.frame(methode=c("")))
```



Pour chaque méthode, on réussi à avoir des accuracy de plus de 98% ce qui paraît logique tant les attaquants et les défenseurs aparaissent comme 2 groupes différents.
...
train_mid = train_mid[,variablesAGarder]

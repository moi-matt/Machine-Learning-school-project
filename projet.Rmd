...
Certains jeunes joueurs peuvent jouer à des postes très différents. Notre objectif est de pouvoir conseiller le poste qui conviendrait le mieux à ces jeunes joueurs. Nous utilisons pour cela des méthodes de machine learning
...
## Classification attaquants défenseurs
...
### I.B SVM et Arbre de décision

Svm linéaire
Le svm consiste à maxim
...
```
On teste le modèle svm pour différents paramètre de pénalisation (le vecteur C).
```{r}
...
train.mod.lin
```
Nous obtenons le meilleur modèle pour C=1. On visualise maintenant la performance de ce modèle 
...
```
On obtient plus de 98% d'accuracy pour C=0.On regarde la performance de notre modèle sur les données tests.
```{r}
mod.final <- svm(Position~.,data=svm_player_omit,kernel="linear",cost=0.1,decision.values=TRUE,probability=TRUE)
prob <- predict(mod.final,newdata=test,probability=TRUE,decision.values=TRUE)
...
prev <- predict(mod.final,newdata=test)
...
**Remarque**
Le stoquage de noyau est très couteux, augmenter le nombre de variable revient à augmenter le temps de calcul de manière quadratique.


Maintenant, on réalise la classification avec le modèle de svm radial. Son noyau est Gaussien
```{r}
C <- c(0.001,0.01,1,10,100,1000)
...
```
Le meilleur modèle d'entrainement est obenu pour C=- et sigma=-1

```{r}
library(e1071)

mod.svm.rad <- svm(Position~.,data=svm_player,kernel="radial",sigma=-1,cost=1)
test$Position <- as.factor(test$Position)
pred.rad = predict(mod.svm.rad, test)
confusionMatrix(pred.rad, reference = test$Position)
```

Nos 2 modèles ont plus de 99% d'accuracy sur les données test





### I.C Arbre de décision 
L'arbre de décision consiste à ...
...
library(plotROC)
```
On compare le svm et l'arbre de décision avec une courbe ROC.
```{r}
...
On peu remarquer su'il n'y a pas de grande différence entre les deux modèles si l'on compare la précision. On peut cependant noter que l'utilisation de l'arbre de décision est beaucoup plus rapide que le svm. 

Les algorithmes d'arbres de décision ne calculent pas tous les arbres possibles lorsqu'ils ajustent un arbre. S'ils le faisaient, ils résoudraient un problème NP-dur. Les algorithmes d'ajustement des arbres de décision prennent généralement des décisions chronophages au cours du processus d'ajustement. En effet, à chaque étape, ils optimisent le sous-problème de recherche avec une division optimale des données d'un noeud. Puis ils continuent à avancer dans le processus d'ajustement. C'est pour cela que notre algorithme de sélection du meilleur arbre est relativement rapide

Maintenant, on essaie d'améliorer l'algorithme avec adaboost
#### Ada boost
...
```{r}
boost.test.data$Position=as.factor(boost.test.data$Position)
...
### I.D Réseaux de neurones

On termine la classification des attaquants et des défenseurs en utilisant les réseaux de neurones. 

Nous testerons des modèles avec différents fonction d'activation.

Si le codes ne s'exécute pas, vous pouvez l'exécuter sur le collab suivant : https://colab.research.google.com/drive/1Fweo0IMDk-nwys8gIQNWlODgUsQJwXmC
...
library(tensorflow)
```
Les 3 fonctions ci-dessous nous permettent de définir des modèles de réseaux de neurones à 1,2 ou 3 couches 
```{r}
...
```
La fonction ci-dessous nous permet de compiler un modèle et de l'exécuter.
```{r}
compile_fit<-function(fun, activation, x=x_train,y=y_train, epochs=30, verbose=0, batch_size=8, validation_split=0.3){
  #Permet de compiler un modèle
...
```
On obtient un très bon résultat avec la fonction d'activation sigmoid. En revanche,   

On teste avec 2 couches pour voir si l'on améliore ou non le résultat.
...
Les modèles avec 2 couches ne sont pas forcément meilleurs. On garde notre modèle à une couche ayant la fonction sigmoid.

### I.D Comparaison des modèles avec uniquement la prise en compte des attaquants et des défenseurs.
...
Pour chaque méthode, on réussi à avoir des accuracy de plus de 98% sur les données test, ce qui paraît logique tant les attaquants et les défenseurs apparaissent comme 2 groupes différents.


Nous allons maintenant réaliser notre classification en prenant en compte les milieux de terrain afin de mieux représenter notre problème.
...
### II.A Régression logistique


## Prise en compte des milieux de terrain

Le modèle logistique en tant que tel ne se limite qu'aux variables réponses binaires. 

Or, dans ce nouveau cas, nous souhaitons mettre en place une régression sur une variable à trois modalités. Il va donc falloir trouver une manière d'adapter la régression logistique à ce cas. Nous allons voir de manière de faire cela.
...
```{r}
train2=train_mid
train2$Position=as.factor(train_mid$Position)

modTest = vglm(Position~.,data = train_mid, family = multinomial)
summary(modTest)
```
...
train2 = train2[,variablesAGarder]
test_mid = test_mid[, variablesAGarder]
modTest = vglm(Position~. ,data = train2, family = multinomial)
summary(modTest)
```
...
head(prediction)
```
On peut représenter pour chacun des joueurs la probabilités qu'ils soient un un défenseur, un milieu ou un attaquant.

```{r}
plot(prediction[,1], main="Probabilité pour chaque joueur d'être défenseur")
plot(prediction[,2], main="Probabilité pour chaque joueur d'être milieu")
plot(prediction[,3], main="Probabilité pour chaque joueur d'être attaquant")
```
On peut voir que les milieux sont plus difficiles à prédire, en effet, on voit que beaucoup de joueur ont une probabilté d'être milieu proche de 0.

On prédit la catégorie du joueur en prenant le maximum des probabilités pour chaque individu.
...
Un deuxième type de modèle découle de l'observation suivante : défenseur, milieu et attaquant ne sont pas simplement trois modalité : on peut leur attacher une notion d'ordre. En effet, pour simplifier, on peut dire que les milieux sont des joueurs "intermédiaires" entre les attaquants et les défenseurs. On peut donc supposer que cette notion d'ordre pourrait se transmettre aussi aux compétences que le joueurs possèdent. 
...
```{r}
pred = as.factor(round(predict.glm(modOrdonne, newdata = test_mid, type = 'response')*2))
verif = as.integer(pred == test_mid$Position)
...
```{r}
tune.3positions.lin <- tune(svm,Position~.,data=train_mid,kernel="linear",ranges=list(cost=c(0.01,0.1,1,10,100,1000)))
...
```{r}
pred.svm.poly <- predict(train.svm.poly,newdata = test_mid)

```

Parmi les 3 noyaux testés, on obtient de bien meilleurs résultats avec le svm polynomial. On conserve donc le modèle polynomial.

### II.C Réseaux de neurones
...
```{r}
compar2=ggplot(data.frame(methode=c("regression polytomique nominale", "regression polytomique ordonnee", "svm polynomial", "neural network"), value=c(0.77,0.76, 0.87,0.84) ))+geom_col(aes(x=methode, y=value))+ggtitle("Comaraison des algorithmes \n en ayant pris uniquement attaquants milieux et défenseurs")
compar2
```
## Conclusion 
